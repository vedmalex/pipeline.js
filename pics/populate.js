...
function exRelationLookup (db, relsContext, rec, relDef, callback) {
  ...
  if (relsContext[relName][_from] && relsContext[relName][_from][_to]) {
    found = relsContext[relName][_from][_to]
  }
        ...
        var linkObj = function(err, toUpdate) {
          if (toUpdate) {
            ...
            if (rel.single) {
              var newVal = dest.to
              if (
                newVal === null || newVal == '$$$unlink$rel$$$' || newVal === ''
              ) {
                newVal = undefined
              }
      ...
      run: function(err, ctx, done) {
        ...
        var relObj =
          ctx
            .saveList[
              ctx.objectId
            ] /*обновить объект на противоположенной стороне ассоциации*/
        var linkObj = function(err, toUpdate) {
          if (toUpdate) {
            ...
            var connect = function(key, remove) {
              if (key === null || key === '$$$unlink$rel$$$' || key === '') {
                key = undefined
              }
              if (key) {
                var opposite = findObjectBy(
                  db,
                  loadFromDB,
                  serverModel,
                  rel.toKeyField,
...
var linkNotEmbedded = new pipeline.Stage({
  ...
  run: function(err, ctx, done) {
    ...
    var to = dest.to
    var nothingLink = to === undefined || to === null
      || to === '$$$unlink$rel$$$' || to === ''
...
var unlinkNotEmbedded = new pipeline.Stage({
  ...
  run: function unlinkNotEmbedded(err, ctx, done) {
    ...
    // ищем связь в базе и помечаем ее на удаление.
    var from = dest.from
    var nothingToUnlink = from === null || from === undefined
      || from === '$$$unlink$rel$$$' || dest.from === ''
...
var update = new pipeline.Pipeline({
  name: 'Update records',
  stages: [prepareChanges, function makeChanges(err, ctx, done) {
...
var processChangeSet = new pipeline.MultiWaySwitch({
  ...
  cases: [{
    evaluate: function(ctx) {
      return ctx.item.action === 'update' || ctx.item.action === 'create'
        || ctx.item.action === 'unlink'
    },
  ...
  new pipeline.Sequential({
    ...
    stage: function preparePreload(err, ctx, done) {
      ...
      for (i = 0; i < len; i++) {
        ...
        for (j = 0; j < allLen; j++) {
          if (objCond[relDef.toKeyField][all[j]] !== -1) {
            objCond[relDef.toKeyField][all[j]] = 1
          }
        }
      }
      // belongsTo
      ...
      for (i = 0; i < len; i++) {
        ...
        var to = rel.to
        if (
          to !== null && to !== undefined && to !== '$$$unlink$rel$$$'
          && to !== ''
        ) {
          if (objCond[relDef.toKeyField][to] !== -1) {
            objCond[relDef.toKeyField][to] = 1
          }
        }

        var from = rel.from
        if (
          from !== null && from !== undefined && from !== '$$$unlink$rel$$$'
          && from !== ''
        ) {
          if (objCond[relDef.toKeyField][from] !== -1) {
            objCond[relDef.toKeyField][from] = 1
          }
        }
      }
      done()
...
function extractTypeName (_v) {
  ...
  if (ret === 'Function') {
    ...
    if (gtype && gtype instanceof Function) {
      ...
      cfg.value = val.slice(1, val.length - 1).replace(/\{/g, '\\{').replace(
        /\}/g,
        '\\}',
      ).replace(/,/g, ',\n').replace(/\[/g, '\\[\n').replace(/\]/g, '\n\\]')
...
while (Objects.length > 0) {
  ...
  if (!described[describedItem]) {
    ...
    if (curr instanceof Function) {
      ...
      getAllMembersList(curr.prototype).forEach(
        describe(curr, describedItem.properties),
      )

      // display items
      console.log(
        describedItem.toString(),
        describedItem.referencedType ? ('-> ' + describedItem.ref) : '',
      )
