...
class Timeout extends stage_1.Stage {
    constructor(config) {
        super();
        if (config) {
            this._config = (0, types_1.getTimeoutConfig)(config);
        }
    }
    get reportName() {
                ...
                to = setTimeout(() => {
                    if (to) {
                        if (this.config.overdue) {
                            (0, run_or_execute_1.run_or_execute)(this.config.overdue, err, ctx, localDone);
                        }
                    }
                }, waitFor);
                if (this.config.stage) {
                    (0, run_or_execute_1.run_or_execute)(this.config.stage, err, ctx, localDone);
                }
            }
            else {
                if (this.config.stage) {
                    (0, run_or_execute_1.run_or_execute)(this.config.stage, err, ctx, done);
                }
            }
        };
        this.run = run;
