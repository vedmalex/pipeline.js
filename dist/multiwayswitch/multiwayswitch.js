    ...
    if (Array.isArray(config)) {
        return {
            cases: config.map(item => {
                ...
                if ((0, types_2.isRunPipelineFunction)(item)) {
                    ...
                    throw (0, CreateError_1.CreateError)(new Error('not suitable type for array in pipelin'));
                }
                return res;
            }),
        };
        ...
        if ((0, types_1.isAnyStage)(res)) {
            ...
            if ((config === null || config === void 0 ? void 0 : config.run) && config.cases && config.cases.length > 0) {
                throw (0, CreateError_1.CreateError)(new Error(" don't use run and stage both "));
            }
            if (config.run) {
            ...
            let next = (index) => {
                return (err, retCtx) => {
                    ...
                    if (iter >= actuals.length) {
                        return done(hasError ? (0, CreateError_1.CreateError)(errors) : undefined, (res !== null && res !== void 0 ? res : ctx));
                    }
                };
            };
            let stg;
