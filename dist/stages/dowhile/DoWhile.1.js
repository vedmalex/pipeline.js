...
const getDoWhileConfig_1 = require("./getDoWhileConfig");
class DoWhile extends stage_1.Stage {
    constructor(config) {
        super();
        if (config) {
            this._config = (0, getDoWhileConfig_1.getDoWhileConfig)(config);
        }
    }
    get reportName() {
        ...
        let run = async (err, context, done) => {
            ...
            const next = async (err) => {
                ...
                while (!this.reachEnd(err, context, iter)) {
                    ;
                    [err, retCtx] = await (0, stage_1.run_or_execute_async)(this.config.stage, err, this.split(context, iter));
                    if (err) {
                        ;
...
//# sourceMappingURL=DoWhile.1.js.map