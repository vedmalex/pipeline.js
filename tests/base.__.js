...
describe('Context', function() {
  it('default empty', function(done) {
    ...
    assert.strictEqual(ctx.__childs === undefined, true, 'MUST BE EMPTY')
    done()
  })
  ...
  it('overwrite', function(done) {
    ...
    assert.strictEqual(c.some, 1)
    assert.strictEqual(c.someOther[0], 1)
    assert.strictEqual(c.someOther.length, 5)
    c.overwrite({
      some: 2,
      thirdParty: 3,
    })
    assert.strictEqual(c.some, 2)
    assert.strictEqual(c.thirdParty, 3)
    assert.strictEqual(c.someOther[0], 1)
    assert.strictEqual(c.someOther.length, 5)
  ...
  it('get', function(done) {
    ...
    assert.strictEqual(ce.some, 1)
    assert.strictEqual(c.embedded.some, 1)
    assert.strictEqual(ce.someOther, c.embedded.someOther)
    assert.strictEqual(ce.deep1.liter, c.embedded.deep1.liter)
  ...
  it('fork child', function(done) {
    ...
    var ctx = new Context(config)
    var ctx2 = ctx.fork()
    assert.strictEqual(ctx.config.some, ctx2.config.some, 'MUST BE strictEqual')
    assert.strictEqual(ctx.notConfig, ctx2.notConfig, 'MUST BE strictEqual')
    assert.strictEqual(ctx2.getParent() === ctx, true, 'parent is context')
    done()
  })
  ...
  it('fork with extra config', function(done) {
    ...
    assert.strictEqual(subCtx.another, 1)
    done()
  })
})
...
describe('Pipeline', function() {
  ...
  it('addStage', function(done) {
    ...
    assert.strictEqual(pipe.stages[0] instanceof Stage, true)
    done()
  })
  ...
  it('addStage converts function to Stage Instance', function(done) {
    ...
    assert.strictEqual(
      pipe.stages[0] instanceof Stage,
      true,
      'function is converted to Stage instance',
    )
    done()
  })
  ...
  it('accept array of stages', function(done) {
    ...
    assert.strictEqual(
      pipe.stages[1] instanceof Stage,
      true,
      'function is converted to Stage instance',
    )
    done()
  ...
  it('compile', function(done) {
    ...
    assert.strictEqual(
      typeof pipe.run,
      'function',
      'must have run-function after compile',
    )
    pipe.addStage(new Stage())
    assert.strictEqual(
      !pipe.run,
      true,
      'after addStage nothing need recompoile',
    )
    done()
  })
  ...
  it('execute must call compile and ensure', function(done) {
    ...
    pipe.execute({})
    assert.strictEqual(ensure, 1, 'must ensure')
    assert.strictEqual(compile, 1, 'must compile')

    pipe.execute({})
    assert.strictEqual(ensure, 2, 'must ensure')
    assert.strictEqual(compile, 1, 'must not compile second time')
    done()
  })
  ...
  it('context catch all errors', function(done) {
    ...
    pipe.execute(ctx1, function(err) {
      ...
      assert.strictEqual(ctx1.s1, true, 's1 pass')
      assert.strictEqual(ctx1.s2, true, 's2 pass')
      assert.strictEqual(ctx1.s3, false, 's3 not passed')
      done()
    })
  })
  ...
  it('pipeline accept only ensure', function(done) {
    ...
    assert.strictEqual(
      p1.ensure != ensure,
      true,
      'not accept ensure as function',
    )
    assert.strictEqual(p1.run != ensure, true, 'not accept run at all')
    var p2 = new Pipeline({
      ensure: ensure,
      run: ensure,
    })
    ...
    assert.strictEqual(p2.run != ensure, true, 'not accept run at all')
    done()
  })
  ...
  it('stage accept ensure, validate and run', function(done) {
    ...
    assert.strictEqual(
      p1.validate != ensure,
      true,
      'accept only run as function',
    )
    assert.strictEqual(p1.run == ensure, true, 'accept only run as function')
    var p2 = new Stage({
      ensure: ensure,
      run: ensure,
      validate: ensure,
    })
    assert.strictEqual(p2.ensure === ensure, true, 'accept ensure')
    assert.strictEqual(p2.validate === ensure, true, 'accept validate')
    assert.strictEqual(p2.run === ensure, true, 'accept run')
    done()
  })
  ...
  it('use schema to override validate', function(done) {
    ...
    assert.strictEqual(replaced, true, 'validate method must be replaced')
    done()
  })
  ...
  it('can do subclassing of Pipeline', function(done) {
    ...
    var p1 = new newPipe()
    assert.strictEqual(p1.stages.length, 3, 'must be by default 3')
    var p2 = new newPipe()
    p2.addStage(new Stage())
    assert.strictEqual(p2.stages.length, 4, 'must be by default 3 + 1 new')
    var p3 = new newPipe()
    assert.strictEqual(p3.stages.length, 3, 'must be by default 3')
    done()
  })
  ...
  it('retry use custom restore and backup', function(done) {
    ...
    st.execute(
      ...
      function(err, ctx) {
        ...
        assert.strictEqual(ctx.backup, 1)
        assert.strictEqual(ctx.restore, 4)
        done()
      },
    )
  })
...
describe('inheritence', function() {
  it('override property', function(done) {
    ...
    clone.age = 15
    assert.strictEqual(context.age, 10, 'age stays the same')
    assert.strictEqual(context.num, '#1001', 'num stays the same')
    assert.strictEqual(
      context.data.toString(),
      new Date(1978, 10, 10).toString(),
      'date stays the same',
    )
    assert.strictEqual(context.context, ctx, 'date stays the same')
...
describe('Utils', function() {
  it('getClass works', function(done) {
    ...
    var p = new Pipeline()
    var c = new Context()
    assert.strictEqual(Util.getClass(v), 'Stage')
    assert.strictEqual(Util.getClass(p), 'Pipeline')
    assert.strictEqual(Util.getClass(c), 'Context')
    done()
  })
})
