...
class DoWhile extends stage_1.Stage {
    ...
    compile(rebuild = false) {
        let run = (err, context, done) => {
            ...
            let next = (err, retCtx) => {
                iter++;
                if (this.reachEnd(err, retCtx !== null && retCtx !== void 0 ? retCtx : context, iter)) {
                    return done(err, retCtx !== null && retCtx !== void 0 ? retCtx : context);
                }
                else {
                    run_or_execute_1.run_or_execute(this.config.stage, err, this.split(retCtx !== null && retCtx !== void 0 ? retCtx : context, iter), next);
                }
            };
            next(err, context);
        };
