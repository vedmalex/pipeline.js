...
function Pipeline(config) {
	...
	} else Stage.call(self);

	self.name = (config && config.name) ? config.name : self.name;
	if (!self.name) {
		self.name = [];
	}
...
PipelineProto.stages = undefined;
...
PipelineProto.compile = function() {
	...
	if (len > 0) {
		...
		self.run = function(err, ctx, done) {
			done(err);
		};
		// throw new Error('ANY STAGE FOUND');
	}
};
