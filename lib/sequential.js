...
Sequential.prototype.combine = function(innerCtx, ctx, childrenCtxList) {};

Sequential.prototype.prepareContext = function(ctx) {
	return ctx;
};
/**
	use this to collect errors for external handling 
	or you can produce new error to break execution.
*/
...
Sequential.prototype.compile = function() {
	...
	var run = function(err, ctx, done) {
		...
		var combine = function(err) {
			if (!err && self.combine) {
				self.combine(innerCtx, ctx, childrenCtxList);
			}
			done(err);
		};
