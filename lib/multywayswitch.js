...
MultiWaySwitch.prototype.compile = function() {
	...
	var run = function(err, ctx, done) {
		...
		var next = function(index) {
			...
			return function(err, retCtx) {
				...
				} else if (cur.combine) cur.combine(ctx, retCtx);

				if (iter >= len)
					return done(hasError ? new ErrorList(errors) : undefined);
			};
		};
		var stg;
		var lctx;
		for (i = 0; i < len; i++) {
			...
			stg.stage.execute(err, lctx, next(i));
		}
	...
	self.run = run;
	MultiWaySwitch.super_.prototype.compile.call(self);
};
 ...
 * @api public
 */
MultiWaySwitch.prototype.execute = function(err, context, callback) {
	...
	MultiWaySwitch.super_.prototype.execute.call(self, err, context, callback);
};
