	...
	if (config) {
		...
		if (typeof(config) === 'string') {
			...
			if (config.name) {
				...
				var match = self._runVal.toString().match(/function\s*(\w+)\s*\(/);

				if (match && match[1]) {
					self.name = match[1];
				} else {
					self.name = self._runVal.toString();
				}
			}
		}
	}
...
Stage.prototype._runVal = 0;
Stage.prototype._run = 0;

Object.defineProperty(Stage.prototype, 'run', {
	get: function() {
		return this._run;
	},
	set: function(val) {
...
Stage.prototype.compileIt = function(val) {
	var self = this;
	var done;

	function handleError(_err, context, callback) {
		...
		switch (len) {
			case 0:
				finishIt(self.rescue(), context, callback);
				break;

			case 1:
				finishIt(self.rescue(_err), context, callback);
				break;

			case 2:
				finishIt(self.rescue(_err, context), context, callback);
				break;

			case 3:
				self.rescue(_err, context, function(err) {
					finishIt(err, context, callback);
				});
				break;

			default:
				finishIt(_err, context, callback);
		}
	}
	...
	function finishIt(err, context, callback) {
		if (callback instanceof Function) {
			callback(err, context);
		}
	}

	function doneIt(err, context, callback) {
		if (err) {
			handleError(err, context, callback);
		} else {
			finishIt(undefined, context, callback);
		}
	}

	var runStage = function(err, context, callback) {
		var done = function(err) {
			doneIt(err, context, callback);
		};
		if (typeof(val) == 'function') {
			if (err) {
				if (val.length == 3) {
					ensureAsync(self, val)(err, context, done);
				} else {
					handleError(err, context, callback);
				}
			} else {
				...
				switch (val.length) {
					case 0:
						ensureSync(context, val, done)();
						break;
					case 1:
						ensureSync(self, val, done)(context);
						break;
					case 2:
						ensureAsync(self, val)(context, done);
						break;
					case 3:
						ensureAsync(self, val)(null, context, done);
						break;
					default:
						handleError(new Error('unacceptable signature'), context, callback);
				}
			}
		} else {
			handleError(new Error(self.reportName() + ' reports: run is not a function'), context, callback);
		}
	};
...
Stage.prototype.execute = function(err, _context, callback) {
	if (!this._run) {
		this._run = this.compileIt(this._runVal);
	}
