...
Stage.prototype.compileItFte = function() {
	var code = f.run({
		self: this
	}, __dirname + '/templates/stage.njs', true);
	this.exec = eval(code);
	this.compiled = true;
};
...
Stage.prototype.compileIt = function(val) {
	...
	var runStage = function(err, context, callback) {
		...
		if (typeof(val) == 'function') {
			if (err) {
				if (val.length == 3) {
					fpAsyncCall(self.handleError.bind(self), self, val)(err, context, done);
				} else {
					self.handleError(err, context, callback);
				}
			} else {
				...
				switch (val.length) {
					case 0:
						fpSyncCall(self.handleError.bind(self), context, val, done)();
						break;
					case 1:
						fpSyncCall(self.handleError.bind(self), self, val, done)(context);
						break;
					case 2:
						fpAsyncCall(self.handleError.bind(self), self, val)(context, done);
						break;
					case 3:
						fpAsyncCall(self.handleError.bind(self), self, val)(null, context, done);
						break;
					default:
...
Stage.prototype.execute = function(err, _context, callback) {
	...
	if (!useFte)
		...
		this.exec(err, _context, callback);
};
