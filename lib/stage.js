...
var withTimeout_1 = require("./utils/async/withTimeout");
var config_1 = require("./config");
exports.StageSymbol = Symbol('stage');
function isStage(obj) {
    return !!(obj === null || obj === void 0 ? void 0 : obj[exports.StageSymbol]);
}
var Stage = (function () {
    ...
    Object.defineProperty(Stage.prototype, "reportName", {
        get: function () {
            return "STG:".concat(this._config.name ? this._config.name : '');
        },
        enumerable: false,
        configurable: true
    });
    ...
    Stage.prototype.runStageMethod = function (err_, err, ctx, context, stageToRun, callback) {
        if (err || err_) {
            var errors = [err, err_].filter(function (e) { return (0, TypeDetectors_1.isError)(e); });
            var combinedError = errors.length > 0 ? (0, ErrorList_1.createError)(errors) : (0, ErrorList_1.createError)('Unknown error occurred');
            if (this.config.run && !(0, can_fix_error_1.can_fix_error)({ run: this.config.run })) {
                this.rescue(combinedError, ctx !== null && ctx !== void 0 ? ctx : context, callback, function (rescuedContext) {
                    stageToRun(undefined, rescuedContext, callback);
                });
            }
            else {
                stageToRun(combinedError, ctx !== null && ctx !== void 0 ? ctx : context, callback);
            }
        }
        else {
    ...
    Stage.prototype.execute = function (_err, _context, _callback) {
        var _this = this;
        var _a;
        var err, not_ensured_context, __callback;
        ...
        var input_is_context = context_1.Context.isContext(not_ensured_context);
        var context = context_1.Context.ensure(not_ensured_context);
        if (input_is_context) {
            ;
            context[context_1.OriginalObject] = true;
        }
        context[context_1.CurrentStage] = this;
        if (!__callback) {
            var _b = Promise.withResolvers(), promise = _b.promise, resolve_1 = _b.resolve, reject_1 = _b.reject;
            this.execute(err, context, (function (err, ctx) {
                if (err)
                    reject_1(err);
                else {
                    if (input_is_context) {
            ...
            var alreadyRun_1 = false;
            var callbackExecuted_1 = false;
            var _c = Promise.withResolvers(), promise = _c.promise, resolve_2 = _c.resolve, reject_2 = _c.reject;
            var back_1 = function (err, _ctx) {
                if (!alreadyRun_1) {
                    try {
                        if (input_is_context) {
                            __callback(err, _ctx);
                            ...
                            if (context_1.Context.isContext(_ctx)) {
                                ...
                                var errors = [err, new Error('context is always context object')].filter(function (e) { return (0, TypeDetectors_1.isError)(e); });
                                __callback(errors.length > 0 ? (0, ErrorList_1.createError)(errors) : (0, ErrorList_1.createError)('Context error occurred'), _ctx);
                            }
                        }
                        resolve_2();
                    }
            ...
            var sucess_1 = function (ret) { return back_1(undefined, ret !== null && ret !== void 0 ? ret : context); };
            var fail_1 = function (err) { return back_1(err, context); };
            var callback_1 = (function (err, _ctx) {
                if (!callbackExecuted_1) {
                    callbackExecuted_1 = true;
                    if (err) {
                        _this.rescue(err, _ctx, fail_1, sucess_1);
                    }
                    else {
                        back_1(err, _ctx);
                    }
            ...
            if (err &&
                this._config.run &&
                !(0, can_fix_error_1.can_fix_error)({ run: this._config.run })) {
                this.rescue(err, context, fail_1, sucess_1);
            }
            else {
                if (this.config.ensure) {
                    this.ensure(this.config.ensure, context, (function (err_, ctx) {
                        _this.runStageMethod(err, err_, ctx, context, stageToRun, callback_1);
                    }));
                }
                    ...
                    stageToRun(undefined, context, callback_1);
                }
            }
            return (0, withTimeout_1.withTimeout)(promise, config_1.Config.timeout).catch(function (originalErr) {
                var error = (0, ErrorList_1.createError)(originalErr || new Error('Stage timeout occurred'));
                if (alreadyRun_1 || callbackExecuted_1) {
                    throw error;
                }
                else {
                    fail_1(error);
                ...
                .finally(function () { return alreadyRun_1 = true; });
        }
    };
    Stage.prototype.stage = function (err, context, callback) {
        var _this = this;
        var back = callback;
        var sucess = function (ret) { return back(undefined, ret !== null && ret !== void 0 ? ret : context); };
        ...
        if (this._config.run) {
            ...
            var errorMessage = this.reportName + ' reports: run is not a function';
            this.rescue((0, ErrorList_1.createError)(errorMessage), context, fail, sucess);
        }
    };
    ...
    Stage.prototype.rescue = function (_err, context, fail, success) {
        var err;
        if (_err) {
            if (!(0, TypeDetectors_1.isError)(_err)) {
                err = (0, ErrorList_1.createError)(_err);
            }
            else {
                err = _err.isClean ? _err : (0, ErrorList_1.createError)(_err);
            }
        }
        else {
            err = null;
