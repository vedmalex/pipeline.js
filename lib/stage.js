...
Stage.prototype.execute = function(_context, callback) {
	...
	if (!process.domain) {
		process.on('uncaughtException', finalizeIt);
	} else {
		process.domain.on('error', finalizeIt);
	}

	self.on('end', function() {
		if (!process.domain) {
			process.removeListener('uncaughtException', finalizeIt);
		} else {
			process.domain.removeListener('error', finalizeIt);
		}
	});
			...
			if (typeof(self.run) == 'function') {
				...
				switch (self.run.length) {
					...
					case 3:
						setImmediate(function() {
							try {
								self.run(null, context, finalizeIt);
							} catch (e) {
								finalizeIt(e);
							}
						});
					...
					default:
						finalizeIt(new Error('unaceptable signature'));
				}
