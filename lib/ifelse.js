...
class IfElse extends stage_1.Stage {
    constructor(config) {
        super();
        if (config) {
            this._config = types_1.getIfElseConfig(config);
        }
    }
    get reportName() {
    ...
    compile(rebuild = false) {
        let run = (err, context, done) => {
            if (typeof this.config.condition == 'function') {
                execute_validate_1.execute_validate(this.config.condition, context, (err, condition) => {
                    if (condition) {
                        if (this.config.success)
                            run_or_execute_1.run_or_execute(this.config.success, err, context, done);
                    }
                    else {
                        if (this.config.failed)
                            run_or_execute_1.run_or_execute(this.config.failed, err, context, done);
                    }
                });
            }
